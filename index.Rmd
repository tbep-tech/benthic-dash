...
load(file = 'data/sedimentdata.RData')
...
load(file = 'data/benspedat.RData')
load(file = 'data/sedspedat.RData')
load(file = 'data/ambiscr_tb.RData')
load(file = 'data/ambipts_tb.RData')
load(file = 'data/ambimed_tb.RData')
load(file = 'data/ambimat_tb.RData')
load(file = 'data/sedplo_default.RData')
load(file = 'data/sedmap_default.RData')

benptsyr <- benpts %>% dplyr::filter(yr == maxyr)
```
...
# AMBI matrix
ambimat_react <- reactive({

  yrsel_ambi <- input$yrsel_ambi

  lwid <- 1.5

  p <- ambimat_tb +
    ggplot2::geom_hline(yintercept = yrsel_ambi - 0.5, lwd = lwid) +
    ggplot2::geom_hline(yintercept = yrsel_ambi + 0.5, lwd = lwid) +
    ggplot2::geom_segment(ggplot2::aes(x = 0.5, xend = 0.5, y = yrsel_ambi - 0.5, yend = yrsel_ambi + 0.5, lwd = lwid)) +
    ggplot2::geom_segment(ggplot2::aes(x = 8.5, xend = 8.5, y = yrsel_ambi - 0.5, yend = yrsel_ambi + 0.5, lwd = lwid))

  tbeptools::show_matrixplotly(p, tooltip = 'Action')

})

ambimap_proxy <- leaflet::leafletProxy('ambimap')
...
observe({
  ...
  ambipol <- ambimed_tb %>%
    dplyr::filter(yr == yrsel_ambi) %>%
    dplyr::inner_join(segs, ., by = 'bay_segment')

  ambipts_yr <- ambipts_tb %>%
    dplyr::filter(yr == yrsel_ambi) %>%
    dplyr::filter(FundingProject == 'TBEP')
  ...
  ambimap_proxy %>%
    ...
    leaflet::addPolygons(
      ...
      fillColor = ~outcome,
      fillOpacity = 0.3,
      label = ~paste0(bay_segment, ': ', long_name, ', AMBI: ', AMBICat)
    ) %>%
    leaflet::addCircleMarkers(
      data = ambipts_yr,
      layerId = ~StationID,
      ...
      fillOpacity = 1,
      radius = 4,
      label = ~paste0('StationNumber ', StationNumber, ', AMBI: ', round(AMBI, 1), ', Category: ', AMBICat)
    )

})

# AMBI summary table
ambitab_react <- reactive({
  yrrng_ambi <- input$yrrng_ambi
  req(yrrng_ambi)
  tbeptools::show_ambitab(ambiscr_tb, yrrng = yrrng_ambi)
})
...
sedplo <- reactive({
  ...
  if(typsel == 'PEL summary'){
    ...
    if(all(yrsel2 == c(1993, maxyr)))
      return(sedplo_default)
...
alplo <- reactive({
  ...
  validate(
    need(typsel == 'Metals', 'Select metals')
  )
  
  out <- try(tbeptools::show_sedimentalratio(sedimentdata, param = prmsel, yrrng = yrsel2, plotly = T), silent = T)
    
  validate(
    need(!inherits(out, 'try-error'), 'Select different metal contaminant')
  )
...
sedmap <- reactive({
  ...
  prmsel <- input$prmsel
  yrsel2 <- input$yrsel2
  # isolate() breaks the cycle: sedmap renders -> fires sedmap_center -> updates
  # sedmaploc() -> would re-trigger sedmap. Reading isolated avoids the loop.
  sedmaploc <- isolate(sedmaploc())
  
  if(typsel == 'PEL summary'){

    if(all(yrsel2 == c(1993, maxyr)) && length(sedmaploc) == 0)
      return(sedmap_default)
...
spedattab <- reactive({
  ...
  out <- reactable::reactable(totab,
    ...
    columns = list(
      Year = reactable::colDef(width = 100),
      Acronym = reactable::colDef(show = F)
    ),
    defaultPageSize = nrow(totab), 
    filterable = TRUE, 
    selection = 'multiple',
    defaultSelected = 1:nrow(totab)
    )
...
# special study site table selection
spedatsel <- reactive({
  
  # inputs
  rowsel <- reactable::getReactableState("spedattab", "selected")
...
spedattxt <- reactive({
  ...
  validate(
    need(rowsel, 'No projects selected')
  )  
  
  nproj <- length(rowsel)

  out <- dplyr::case_when(
    nproj == nrow(spedat) ~ 'All projects selected', 
    nproj == 1 ~ '1 project selected', 
    T ~ paste(nproj, 'projects selected')
  )
...
# sediment special study site selection
sedspedatsel <- reactive({
  
  # inputs
  spedatsel <- spedatsel()
  
  # get data subset and year range
  dat <- sedspedat %>% 
    dplyr::filter(yr %in% spedatsel$Year & Acronym %in% spedatsel$Acronym)
  yrrng <- range(dat$yr)
...
# benthic special study site selection
benspedatsel <- reactive({
...
sedspeplo <- reactive({
  ...
  sedspedatsel <- sedspedatsel()
  
  dat <- sedspedatsel$dat
  yrrng <- sedspedatsel$yrrng
  
  if(typsel2 == 'PEL summary'){
    
    out <- try(tbeptools::show_sedimentpelave(dat, yrrng = yrrng, plotly = T, funding_proj = 'TBEP-Special', ylim = c(0, NA)), silent = T)
  ...
  if(typsel2 != 'PEL summary'){
    ...
    out <- try(tbeptools::show_sedimentave(dat, param = prmsel2, yrrng = yrrng, plotly = T, funding_proj = 'TBEP-Special'), silent = T)
...
# special study aluminum ratio plot
spealplo <- reactive({
  ...
  validate(
    need(typsel2 == 'Metals', 'Select metals')
  )
  
  out <- try(tbeptools::show_sedimentalratio(dat, param = prmsel2, yrrng = yrrng, funding_proj = 'TBEP-Special', plotly = T), silent = T)
...
sedspemap <- reactive({
  ...
  if(typsel2 == 'PEL summary'){
    ...
    out <- tbeptools::show_sedimentpelmap(dat, yrrng = yrrng, funding_proj = 'TBEP-Special')
  ...
  if(typsel2 != 'PEL summary'){
    ...
    out <- try(tbeptools::show_sedimentmap(dat, param = prmsel2, yrrng = yrrng, funding_proj = 'TBEP-Special'), silent = T)
  ...
  isolate({
    sedspemaploc_val <- mapLocs$sedspemaploc
    if(!is.null(sedspemaploc_val) & is.null(mapLocs$sedspezom)){
      out <- out %>%
        leaflet::fitBounds(lng1 = sedspemaploc_val$lng1, lat1 = sedspemaploc_val$lat1, lng2 = sedspemaploc_val$lng2, lat2 = sedspemaploc_val$lat2)
    }
  })

  sedspezom_val <- mapLocs$sedspezom
  if(!is.null(sedspezom_val) & isolate(is.null(mapLocs$sedspemaploc))){
    out <- out %>%
      leaflet::fitBounds(lng1 = sedspezom_val[1], lat1 = sedspezom_val[2], lng2 = sedspezom_val[3], lat2 = sedspezom_val[4])
  }
...
benspemap <- reactive({
  ...
  isolate({
    benspemaploc_val <- mapLocs$benspemaploc
    if(!is.null(benspemaploc_val) & is.null(mapLocs$benspezom)){
      out <- out %>%
        leaflet::fitBounds(lng1 = benspemaploc_val$lng1, lat1 = benspemaploc_val$lat1, lng2 = benspemaploc_val$lng2, lat2 = benspemaploc_val$lat2)
    }
  })

  benspezom_val <- mapLocs$benspezom
  if(!is.null(benspezom_val) & isolate(is.null(mapLocs$benspemaploc))){
    out <- out %>%
      leaflet::fitBounds(lng1 = benspezom_val[1], lat1 = benspezom_val[2], lng2 = benspezom_val[3], lat2 = benspezom_val[4])
  }
...
# shared reactive state for special study map pan/zoom positions
mapLocs <- reactiveValues(
  sedspemaploc = NULL,
  benspemaploc = NULL,
  sedspezom    = NULL,
  benspezom    = NULL
...
observeEvent(input$sedspemap_bounds, {
  mapLocs$sedspemaploc <- list(
    lng1 = input$sedspemap_bounds$west,
    lat1 = input$sedspemap_bounds$south,
    lng2 = input$sedspemap_bounds$east,
    lat2 = input$sedspemap_bounds$north
  )
  mapLocs$sedspezom <- NULL
})

observeEvent(input$benspemap_bounds, {
  mapLocs$benspemaploc <- list(
    lng1 = input$benspemap_bounds$west,
    lat1 = input$benspemap_bounds$south,
    lng2 = input$benspemap_bounds$east,
    lat2 = input$benspemap_bounds$north
  )
  mapLocs$benspezom <- NULL
})
...
sedspebbx <- reactive({
  ...
  # get bbox of sed input    
  out <- sedspedatsel$dat %>% 
    dplyr::select(Longitude, Latitude) %>% 
    unique %>% 
    sf::st_as_sf(coords = c('Longitude', 'Latitude'), crs = 4326)
  
  # buffer if only one point
  if(nrow(out) == 1)
    out <- out %>% 
...
observeEvent(input$zoomto, {
  sedspebbx_val <- sedspebbx()
  mapLocs$sedspezom    <- sedspebbx_val
  mapLocs$benspezom    <- sedspebbx_val
  mapLocs$benspemaploc <- NULL
  mapLocs$sedspemaploc <- NULL
})
...
dldat <- reactive({
  ...
  out <- switch(typseldl,
    'TBBI bay segment' = benmed %>%
      dplyr::select(-outcome),
    'TBBI stations' = tbbiscr %>%
      dplyr::select(-ProgramID) %>%
      dplyr::filter(AreaAbbr %in% c("OTB", "HB", "MTB", "LTB", "TCB", "MR", "BCB")),
    'AMBI bay segment' = {
      props <- tbeptools::anlz_ambimed(ambiscr_tb)
      dplyr::left_join(props,
        dplyr::select(ambimed_tb, bay_segment, yr, AMBICat),
        by = c('bay_segment', 'yr'))
    },
    'AMBI stations' = ambiscr_tb %>%
      dplyr::rename(AMBI = TBAMBI, AMBICat = TBAMBICat) %>%
      dplyr::filter(AreaAbbr %in% c("OTB", "HB", "MTB", "LTB", "TCB", "MR", "BCB")) %>%
      dplyr::select(StationID, AreaAbbr, FundingProject, ProgramName, Latitude, Longitude, date, yr, AMBI, AMBICat),
    'PEL summary' = pelsum,
    {
      req(prmseldl)
      sedimentdata %>%
...
1) [__AMBI__](#ambi): View annual summaries by major bay segments for the Tampa Bay AZTI Marine Biotic Index (AMBI)
...
1) [__DATA DOWNLOADS__](#data-downloads): Download TBBI scores, AMBI-TB scores, or sediment contaminant data
...
The TBBI provides an estimate of the health of benthic communities in Tampa Bay. The TBBI is scaled from 0-100 with values <73 classified as <span style="color:#CC3231">__Degraded__</span>, from 73-87 as <span style="color:#E9C318">__Intermediate__</span>, and >87 as <span style="color:#2DC938">__Healthy__</span>. These classifications are assigned to individual sampling stations in Tampa Bay. Collectively, bay segments are assigned to categories of <span style="color:#CC3231">__Poor__</span>, <span style="color:#E9C318">__Fair__</span>, and <span style="color:#2DC938">__Good__</span> based on the distribution of classifications across all stations in each segment. Bay segment results are based on a five-year rolling assessment starting from 2005 to account for reduced sampling effort. This information provides a general summary of benthic community health across the bay.  These results are shown on the [__TAMPA BAY BENTHIC INDEX__](#tampa-bay-benthic-index) tab. 

#### AZTI Marine Biotic Index

The AZTI Marine Biotic Index (AMBI) is a complementary index of benthic community health based on the ecological sensitivity of taxa to organic enrichment. Species are assigned to one of five ecological groups ranging from very sensitive to very tolerant of disturbance, and the AMBI score is computed as a weighted mean of those group abundances. AMBI scores range from 0 to 10, where lower values indicate better conditions, and individual stations are classified into five categories:

* <span style="color:#2DC938">__Unpolluted__</span>: dominated by sensitive species
* <span style="color:#8DBE68">__Slightly Polluted__</span>: sensitive species present with some tolerant taxa
* <span style="color:#E9C318">__Meanly Polluted__</span>: indifferent and tolerant species increasing
* <span style="color:#E07B39">__Heavily Polluted__</span>: second-order opportunists dominant
* <span style="color:#CC3231">__Extremely Polluted__</span>: first-order opportunists dominant

This dashboard uses a Tampa Bay variant of the AMBI, which applies ecological group assignments calibrated specifically for Tampa Bay species rather than the original Gulf of Mexico classification. These results are shown on the [__AMBI__](#ambi) tab.
...
The __MATRIX RESULTS__ show the TBBI scores each year for each segment, which are based on the distribution of site scores in each segment. Bay segment results are based on a five-year rolling assessment starting from 2005 to account for reduced sampling effort. The black rectangle shows the selected year from the year slider above the map. The bay segments shown at the top of the matrix are as follows:
...
2 AMBI {data-navmenu="PAGE SELECTION"}
===========================================================
...
```{r}
output$ambimat <- plotly::renderPlotly(ambimat_react())
plotly::plotlyOutput('ambimat')
```

### TREND PLOT

```{r}
selectInput('ambi_segs', 'Select bay segment(s):', multiple = TRUE,
  choices = c('HB', 'OTB', 'MTB', 'LTB', 'TCB', 'MR', 'BCB'),
  selected = c('HB', 'OTB', 'MTB', 'LTB', 'TCB', 'MR', 'BCB'),
  width = '100%')
ambitrend_react <- reactive({
  segs <- input$ambi_segs
  if (is.null(segs) || length(segs) == 0) segs <- c('HB', 'OTB', 'MTB', 'LTB', 'TCB', 'MR', 'BCB')
  tbeptools::show_ambitrend(ambiscr_tb, bay_segment = segs, plotly = TRUE)
})
output$ambitrend <- plotly::renderPlotly(ambitrend_react())
plotly::plotlyOutput('ambitrend')
```

### SUMMARY TABLE

```{r}
sliderInput('yrrng_ambi', 'Select year range:', min = 1993, max = maxyr,
  value = c(1993, maxyr), step = 1, sep = '', width = '100%')
output$ambitab <- renderUI({
  flextable::htmltools_value(ambitab_react())
})
uiOutput('ambitab')
```
...
This tab shows the AZTI Marine Biotic Index (AMBI) for benthic communities sampled across Tampa Bay. The AMBI is an index of ecological quality based on the sensitivity of benthic taxa to pollution, ranging from 0 to 10 where higher values indicate healthier conditions. Individual station scores are assigned to one of five categories:

* <span style="color:#CC3231">__Extremely Polluted__</span>: AMBI score < 1.39
* <span style="color:#E07B39">__Heavily Polluted__</span>: AMBI score 1.39 – 2.89
* <span style="color:#E9C318">__Meanly Polluted__</span>: AMBI score 2.89 – 5.29
* <span style="color:#8DBE68">__Slightly Polluted__</span>: AMBI score 5.29 – 8.29
* <span style="color:#2DC938">__Unpolluted__</span>: AMBI score > 8.29

This tab uses a Tampa Bay variant of the AMBI, which uses ecological group assignments calibrated for local conditions rather than the broader Gulf of Mexico classification.

The __MATRIX RESULTS__ show the dominant AMBI-TB category per bay segment and year, based on the distribution of station scores. Bay segment results use a five-year rolling assessment starting from 2005. The selected year is highlighted with a black rectangle. Bay segments shown are:
...
Placing the cursor over the matrix will show the dominant category for each segment and year.

```{r, out.width='40%', fig.align='center'}
knitr::include_graphics('www/ambimatrixex.png')
```
<br>

In the above example, the dominant category for Hillsborough Bay in 1995 was <span style="color:#8DBE68">__Slightly Polluted__</span>.

The __TREND PLOT__ shows mean annual AMBI scores by bay segment over time, with colored background zones corresponding to the five pollution categories.  Select one to all bay segments to show average AMBI scores for those segments.  Placing the cursor over the plot will show the mean AMBI score for the segment selection(s) and year.  Note that the means or individual bay segments may differ slightly from the bay segment category shown in the matrix and map, which is based on the plurarity of station categories rather than the mean score.

```{r, out.width='30%', fig.align='center'}
knitr::include_graphics('www/ambitrend.png')
```
<br>

The __SUMMARY TABLE__ shows the percentage of stations in each category per bay segment for the selected year range, controlled by the slider above the table.

The __MAP SUMMARY__ on the right shows segment polygons shaded by dominant AMBI category and individual station scores as colored points for the selected year.  Note that some stations may be shown as black points, meaning "azoic" or no organisms were found at these stations.

Placing the cursor over the segment or station will reveal additional information.

```{r, out.width='50%', fig.align='center'}
knitr::include_graphics('www/ambimapex.png')
```
<br>
...
```{r}
ambimapbs_pol <- ambimed_tb %>%
  dplyr::filter(yr == maxyr) %>%
  dplyr::inner_join(segs, ., by = 'bay_segment')

ambimapbs_pts <- ambipts_tb %>%
  dplyr::filter(yr == maxyr) %>%
  dplyr::filter(FundingProject == 'TBEP')

ambimapbs <- base_leaflet() %>%
  leaflet::fitBounds(lng1 = bbx[1], lng2 = bbx[3], lat1 = bbx[2], lat2 = bbx[4]) %>%
  leaflet::addLegend("topright",
    labels = names(ambi_cols),
    colors = unname(ambi_cols),
    title = "AMBI Category", opacity = 1
  ) %>%
  leaflet::addPolygons(
    data = ambimapbs_pol,
    stroke = T,
...
output$ambimap <- leaflet::renderLeaflet(ambimapbs)

fillCol(flex = c(NA, 1),
  column(12,
    column(9,
      sliderInput('yrsel_ambi', 'Select year:', min = 1993, max = maxyr, value = maxyr, step = 1, sep = '', width = '100%', animate = T)
    )
  ),
  leaflet::leafletOutput('ambimap')
)
```

3 SEDIMENT CONTAMINANTS {data-navmenu="PAGE SELECTION"}
===========================================================
...
### METAL:ALUMINUM RATIO

```{r}
output$alplo <- plotly::renderPlotly(alplo())
plotly::plotlyOutput('alplo')
```
...
The __SEDIMENT SUMMARY RESULTS__ shows a summary of PEL ratio summaries or selected contaminants by bay segment.  The summaries show the average and 95% confidence interval, with bay segment abbreviations as follows:
...
The __METAL:ALUMINUM RATIO__ tab shows a plot of concentrations for a selected metal parameter against aluminum concentrations. Aluminum is a common metal in sediments and showing the ratio of a contaminant to aluminum provides a comparison relative to background concentrations. The black lines show the linear fit of a log-log model and the 95% prediction intervals.  Lines for the TEL/PEL values for the selected contaminant are shown, if available, and the points are colored and keyed by bay segment. Metal concentrations for the points in each bay segment that are above the confidence intervals from the black ratio line can be considered higher than "normal" given background aluminum concentrations.  Points that are above the confidence interval and the TEL/PEL values are of particular note.  Points by bay segment can be turned off or on by clicking the legend item. Placing the cursor over a point on the plot will show additional information. 

<br>
```{r, out.width='30%', fig.align='center'}
knitr::include_graphics('www/alratioex.png')
```
<br>
...
fillCol(flex = c(NA, 1),
  column(12,
    ...
    column(3,
      param_selector_ui(input, 'typsel', 'prmsel', prmlkup)
    ),
    column(6,
...
4 SPECIAL STUDY SITES {data-navmenu="PAGE SELECTION"}
===========================================================
...
```{r}
output$spealplo <- plotly::renderPlotly(spealplo())
plotly::plotlyOutput('spealplo')
```
...
Results for the special study sites are shown in the three tabs on the left, where a table of sites each year is shown in __SPECIAL STUDY SITES__, summarized results are shown in __SEDIMENT SUMMARY RESULTS__, and metal to aluminum ratio plots are shown in __METAL:ALUMINUM RATIO__.  The maps on the right show sediment contaminant summaries in the left map and TBBI scores in the right map.  By default, all special study sites for all projects are shown in the plots and maps.  Sites for individual projects can be selected using the check boxes in the table on the __SPECIAL STUDY SITES__ tab.  
...
The __METAL:ALUMINUM RATIO__ tab shows a plot of concentrations for a selected metal parameter against aluminum concentrations. Aluminum is a common metal in sediments and showing the ratio of a contaminant to aluminum provides a comparison relative to background concentrations. The black lines show the linear fit of a log-log model and the 95% prediction intervals.  Lines for the TEL/PEL values for the selected contaminant are shown, if available, and the points are colored and keyed by bay segment.  Points by bay segment can be turned off or on by clicking the legend item. Placing the cursor over a point on the plot will show additional information. 
...
```{r}
output$spedattxt <- renderText(spedattxt())
...
fillCol(flex = c(NA, 1),
  column(12,
    ...
    column(3,
      param_selector_ui(input, 'typsel2', 'prmsel2', prmlkup)
    ),
    column(3,
      textOutput('spedattxt')
    ), 
    column(3, 
      actionButton("zoomto", "Zoom to features")
    )
  ),
  fillRow(
...
5 DATA DOWNLOADS {data-navmenu="PAGE SELECTION"}
===========================================================
...
fillCol(flex =c(NA, 1, NA),
  column(12,
    column(3, 
      selectInput('typseldl', 'Select type:', choices = c('TBBI bay segment', 'TBBI stations', 'AMBI bay segment', 'AMBI stations', 'PEL summary', c(unique(prmlkup$SedResultsType))))
    ),
    column(3,
      param_selector_ui(input, 'typseldl', 'prmseldl', prmlkup,
                        exclude = c('TBBI bay segment', 'TBBI stations', 'AMBI bay segment', 'AMBI stations', 'PEL summary'))
    ),
    column(6,
...
__AMBI bay segment__: AZTI Marine Biotic Index (AMBI-TB) proportion of stations in each pollution category and dominant category by bay segment
...
* *bay_segment*: Abbreviated name for the bay segment
     ...
     * *BCB*: Boca Ciega Bay
* *yr*: Year of the survey
* *Extremely Polluted*: Proportion of stations in the segment categorized as Extremely Polluted
* *Heavily Polluted*: Proportion of stations in the segment categorized as Heavily Polluted
* *Meanly Polluted*: Proportion of stations in the segment categorized as Meanly Polluted
* *Slightly Polluted*: Proportion of stations in the segment categorized as Slightly Polluted
* *Unpolluted*: Proportion of stations in the segment categorized as Unpolluted
* *AMBICat*: Dominant AMBI-TB pollution category for the segment

__AMBI stations__: AZTI Marine Biotic Index (AMBI-TB) scores by sampling station

* *StationID*: Unique station ID
* *AreaAbbr*: Abbreviated name for the bay segment (same abbreviations as above)
* *FundingProject*: Project name responsible for funding the sampling
* *ProgramName*: Name of the monitoring program (Benthic Monitoring is routine)
* *Latitude*: Latitude of the station in decimal degrees, EPSG 4326
* *Longitude*: Longitude of the station in decimal degrees, EPSG 4326
* *date*: Date of the sample
* *yr*: Year of the sample
* *AMBI*: AMBI-TB score (0 to 7, where lower values indicate less pollution)
* *AMBICat*: AMBI-TB pollution category (Unpolluted, Slightly Polluted, Meanly Polluted, Heavily Polluted, or Extremely Polluted)
