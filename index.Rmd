...
alplo <- reactive({
  ...
  validate(
    need(!inherits(out, 'try-error') & !is.null(out), 'Select different metal contaminant')
  )
...
The __METAL:ALUMINUM RATIO__ tab shows a plot of concentrations for a selected metal parameter against aluminum concentrations. Aluminum is a common metal in sediments and showing the ratio of a contaminant compared to aluminum provides a comparison relative to background concentrations, i.e., higher ratios suggest levels greater than background. The black lines show the linear fit of a log-log model and the 95% prediction intervals.  Lines for the TEL/PEL values for the selected contaminant are shown, if available, and the points are colored and keyed by bay segment.  Points by bay segment can be turned off or on by clicking the legend item. Placing the cursor over a point on the plot will show additional information. 
<br>
```{r, out.width='30%', fig.align='center'}
