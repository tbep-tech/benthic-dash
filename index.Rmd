...
sedplo <- reactive({
  ...
  if(typsel == 'PEL summary'){
    ...
    out <- show_sedimentpelave(sedimentdata, yrrng = yrsel2, plotly = T)
  ...
  if(typsel != 'PEL summary'){
    ...
    out <- show_sedimentave(sedimentdata, param = prmsel, yrrng = yrsel2)
...
sedmap <- reactive({
  ...
  if(typsel == 'PEL summary'){
    ...
    out <- show_sedimentpelmap(sedimentdata, yrrng = yrsel2)
  ...
  if(typsel != 'PEL summary'){
    ...
    out <- try(show_sedimentmap(sedimentdata, param = prmsel, yrrng = yrsel2), silent = TRUE)
    validate(
      need(inherits(out, 'leaflet'), paste('No TEL/PEL data for', prmsel))
    )
...
```{r}
output$sedplo <- renderPlotly(sedplo())
plotlyOutput('sedplo')
```
...
fillCol(flex = c(NA, 1),
  column(12,
    column(3, 
      selectInput('typsel', 'Select type:', choices = c('PEL summary', 'Organics', 'Metals', 'Physical', 'Other'))
    ),
    column(3, 
      renderUI({
        typsel <- input$typsel
