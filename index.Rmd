...
load(file = 'data/ambiscr.RData')
load(file = 'data/ambiscr_tb.RData')
load(file = 'data/ambipts.RData')
load(file = 'data/ambipts_tb.RData')
load(file = 'data/ambimed.RData')
load(file = 'data/ambimed_tb.RData')
load(file = 'data/ambimat.RData')
...
ambimat_react <- reactive({
  ...
  ambi_type  <- input$ambi_type

  lwid <- 1.5
  base_mat <- if(ambi_type == 'AMBI-TB') ambimat_tb else ambimat
...
observe({
  ...
  pts_dat <- if(ambi_type == 'AMBI-TB') ambipts_tb else ambipts
  pol_dat <- if(ambi_type == 'AMBI-TB') ambimed_tb  else ambimed
...
ambitab_react <- reactive({
  yrsel_ambi <- input$yrsel_ambi
  ambi_type  <- input$ambi_type
  dat <- if(ambi_type == 'AMBI-TB') ambiscr_tb else ambiscr
  tbeptools::show_ambitab(dat, yrrng = c(yrsel_ambi, yrsel_ambi))
})
...
```{r}
output$ambitrend <- plotly::renderPlotly(
  tbeptools::show_ambitrend(ambiscr, ambiscr_tb, plotly = TRUE)
)
...
output$ambitab <- renderUI({
  ft <- ambitab_react()
  htmltools::HTML(flextable::htmltools_value(ft))
})
uiOutput('ambitab')
```
...
This tab shows the AZTI Marine Biotic Index (AMBI) for benthic communities sampled across Tampa Bay. The AMBI is an index of ecological quality based on the sensitivity of taxa to pollution, ranging from 0 to 10 where higher values indicate healthier conditions. Individual station scores are assigned to one of five categories:
...
Two variants of the index are available. The __AMBI__ variant uses ecological group assignments from Gillett et al. (2015) developed for the Gulf of Mexico. The __AMBI-TB__ variant uses Tampa Bay-specific ecological group assignments calibrated for local conditions. The variant selector above the map controls which scores are shown in the __MATRIX RESULTS__, __SUMMARY TABLE__, and map. The __TREND PLOT__ always shows both variants together for comparison.

The __MATRIX RESULTS__ show the dominant AMBI category per bay segment and year, based on the distribution of station scores. Bay segment results use a five-year rolling assessment starting from 2005. The selected year is highlighted with a black rectangle. Bay segments shown are:
...
The __TREND PLOT__ shows mean annual AMBI scores by bay segment over time, with colored background zones corresponding to the five pollution categories. The __SUMMARY TABLE__ shows the percentage of stations in each category per bay segment for the selected year.

The __MAP SUMMARY__ on the right shows segment polygons shaded by dominant AMBI category and individual station scores as colored points for the selected year.
...
fillCol(flex = c(NA, 1),
  column(12,
    column(6,
      sliderInput('yrsel_ambi', 'Select year:', min = 1993, max = maxyr, value = maxyr, step = 1, sep = '', width = '200%', animate = T)
    ),
    column(3,
      selectInput('ambi_type', 'Select variant:', choices = c('AMBI-TB', 'AMBI'))
    )
  ),
  ...
  column(12,
    ...
    column(3,
      param_selector_ui(input, 'typseldl', 'prmseldl', prmlkup,
                        exclude = c('TBBI bay segment', 'TBBI stations', 'PEL summary'))
    ),
    column(6,
