...
# selected rows of special study site table
spedatsel <- reactive(reactable::getReactableState("spedattab", "selected"))
...
spealplo <- reactive({
  ...
  out <- try(tbeptools::show_sedimentalratio(sedspedat, param = prmsel2, yrrng = yrsel3, funding_proj = 'TBEP-Special', plotly = T), silent = T)
...
sedspeplo <- reactive({
  ...
  if(typsel2 == 'PEL summary'){
    browser()
    out <- try(tbeptools::show_sedimentpelave(sedspedat, yrrng = yrsel3, plotly = T, funding_proj = 'TBEP-Special', ylim = c(0, NA)), silent = T)
  ...
  if(typsel2 != 'PEL summary'){
    ...
    out <- try(tbeptools::show_sedimentave(sedspedat, param = prmsel2, yrrng = yrsel3, plotly = T, funding_proj = 'TBEP-Special'), silent = T)
...
sedspemap <- reactive({
  ...
  if(typsel2 == 'PEL summary'){
    ...
    out <- tbeptools::show_sedimentpelmap(sedspedat, yrrng = yrsel3, funding_proj = 'TBEP-Special')  
  ...
  if(typsel2 != 'PEL summary'){
    ...
    out <- try(tbeptools::show_sedimentmap(sedspedat, param = prmsel2, yrrng = yrsel3, funding_proj = 'TBEP-Special'), silent = T)
...
```{r}
output$spedatsel <- renderPrint(print(spedatsel()))
output$spedattab <- reactable::renderReactable(spedattab())
fluidRow(heights = c(1, NA),
  shiny::verbatimTextOutput('spedatsel'),
