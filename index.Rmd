...
sedspeplo <- reactive({
  ...
  spedat <- sedimentdata %>% 
    dplyr::filter(FundingProject %in% 'TBEP-Special')
  
  if(typsel2 == 'PEL summary'){
    
    out <- tbeptools::show_sedimentpelave(spedat, yrrng = yrsel3, plotly = T)
  ...
  if(typsel2 != 'PEL summary'){
    ...
    out <- tbeptools::show_sedimentave(spedat, param = prmsel2, yrrng = yrsel3, plotly = T)
...
sedspemap <- reactive({
  ...
  prmsel2 <- input$prmsel2
  yrsel3 <- input$yrsel3
  sedspemaploc <- isolate(sedspemaploc()) # fucking important!
  ...
  if(typsel2 == 'PEL summary'){
    ...
    out <- tbeptools::show_sedimentpelmap(spedat, yrrng = yrsel3)  
  ...
  if(typsel2 != 'PEL summary'){
    ...
    out <- tbeptools::show_sedimentmap(spedat, param = prmsel2, yrrng = yrsel3)
...
```{r}
output$sedplo <- plotly::renderPlotly(sedspeplo())
...
fillCol(flex = c(NA, 1),
  column(12,
    ...
    column(6,
      sliderInput('yrsel3', 'Select year:', min = 2002, max = maxyr, value = maxyr, step = 1, sep = '', width = '200%'),
    )
  ),
